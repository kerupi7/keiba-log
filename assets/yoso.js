...
(async function () {
  ...
  // 仕様: 部署/競馬部/handoff_2026-09-24_apt-card.md（試作は mockup-173-apt-screen.html）
  ...
  function aptCard(h) {
    ...
    const cellRow = (label, c, cls) => {
      ...
      return `<div class="ax-r${c && c.n ? '' : ' zero'}${k ? ' ' + k : ''}${cls ? ' ' + cls : ''}">
        <span class="ax-l">${esc(label)}</span>
    ...
    const turnBar = !turnSum ? '' : `<div class="ax-g"><div class="ax-gt">回り（${esc(R.surface)}）　数字は好走率</div>
      <div class="ax-sp">${turn.map((c) => {
        ...
        return `<span class="ax-spc ${k}${c.today ? ' today' : ''}" style="width:${(c.n / turnSum) * 100}%">
          <b>${esc(c.label)}</b><i class="bt-num">${axPct(c)}%</i><u class="bt-num">${c.n}走</u></span>`;
      }).join('')}</div></div>`;
    return `<div class="h-card b-crd ax">
      ...
      <div class="ax-g"><div class="ax-gt">距離（全場・${esc(R.surface)}）　目盛りは100m単位・▲＝今日</div>
        <div class="ax-mt">
